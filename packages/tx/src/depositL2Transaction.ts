...
export class DepositL2Transaction extends BaseTransaction<TransactionType.DepositL2> {
  ...
  public static fromSerializedTx(serialized: Uint8Array, opts: TxOptions = {}) {
    if (equalsBytes(serialized.subarray(0, 1), txTypeBytes(TransactionType.DepositL2)) === false) {
      throw new Error(
  ...
  public static fromValuesArray(values: TxValuesArray, opts: TxOptions = {}) {
    if (values.length < 8) {
      throw new Error('Invalid DepositL2 transaction values array length.')
    }

    const [sourceHash, from, to, mint, value, gasLimit, isSystemTx, data] = values
  ...
  public constructor(txData: TxData, opts: TxOptions = {}) {
    ...
    this.from =
      txData.from !== undefined
        ? new Address(toBytes(txData.from))
        : new Address(new Uint8Array(20))
    this.isSystemTx =
      txData.isSystemTx !== undefined ? bytesToBigInt(toBytes(txData.isSystemTx)) : BigInt(0)
